...
Part 6: adds space.launch/thrust/navigate/telemetry/land (space.orbit
...
space.navigate / space.telemetry / space.land (Part 6).
...
static ZeyValue zey_space_land(ZeyValue coord) {
    printf("[space.land] landing at ");
...
class Compiler:
    ...
    def _compile_binop(self, node: BinOp):
        if node.op == "and":
            return f"ZEY_AND({self.compile_expr(node.left)}, {self.compile_expr(node.right)})"
        if node.op == "or":
            return f"ZEY_OR({self.compile_expr(node.left)}, {self.compile_expr(node.right)})"
    ...
    def _compile_call(self, node: Call):
        ...
        if isinstance(callee, Attribute) and isinstance(callee.obj, Identifier):
            ...
            if ns == "space" and method == "land":
                self._check_arity("space.land", args, 1)
                return f"zey_space_land({self.compile_expr(args[0])})"
            ...
            raise CompileError(
                ...
                "space.navigate, space.telemetry, space.land)"
            )
