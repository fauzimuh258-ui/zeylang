...
Part 6: adds the space library (launch/thrust/navigate/telemetry/touchdown)
...
Part 7: all namespace calls use `@` instead of `.` now (ai@chat(), not
ai.chat()) — this file only changed cosmetically (print prefixes, error
text, docstrings): the actual `.` -> `@` switch lives in lexer.py/parser.py,
since this file just evaluates whatever AST the parser handed it. Also
renamed space.land() -> space@touchdown() (an actual rename, not just
notation) per the Part 7 request.
...
class _Space:
    """All simulated. orbit/launch/thrust/navigate/touchdown print a
